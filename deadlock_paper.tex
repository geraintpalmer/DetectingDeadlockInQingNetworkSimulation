...
The five networks for which Markov models are given are:
\begin{itemize}
...
In \cite{onvural90} three types of blocking are described:
...
In Type III blocking (repetitive services, RS-FD and RS-RD, rejection blocking) instead of getting blocked a customer is required to repeat thier service if there is no capacity at theor destination. This type of blocking comes in two flavours, fixed destination where the customer's destination does not change at each repitition of service, and random destination, where the customer's destination is resamples from a probability distribution after each repitition.
...
\subsubsection{Deadlock Detection \& Recovery}
...
\begin{figure}[!htbp]
  ...
  \begin{subfigure}{\textwidth}
    \begin{center}
      \includestandalone[width=0.7\textwidth]{images/exampledigraph_4}
    \end{center}
    \caption{A three node queueing network in deadlock, with state digraph.}
    \label{fig:exampledigraph_nodeadlock}
    \vspace{6 mm}
