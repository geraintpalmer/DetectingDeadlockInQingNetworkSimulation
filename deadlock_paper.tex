...
A common deadlock avoidance technique in flexible manufacturing systems is the use of dynamic resource allocation policies using petri net models of the system \cite{viswanadhametal90, ezpeletaetal02}.
Some resource allocations are disallowed if that allocation will lead to deadlock.
...
In order to detect when deadlock has occurred in a queueing network simulation,
a state digraph is used.
...
\begin{itemize}
  ...
  \item Consider the case when the server corresponding to $X$ is occupied by
      individual $a$, whose next destination is node $j$. Then $X$'s direct
      successors correspond to servers occupied by individuals who are blocked or in service at node $j$.
  \item It can interpreted that all $X$'s descendants correspond to the servers whose occupants are directly or indirectly blocking $a$, and interpret all $X$'s ancestors as those servers whose individuals who are being blocked directly or indirectly by $a$.
  \item All vertices of $G(t)$ are either descendants of another vertex and so
      corresponds to a servers who are occupied by an individual who is blocking someone; or are ancestors of another vertex, and so are occupied by someone who is blocked.
...
Node $A$ has two parallel server, node $B$ has three parallel servers.
...
% I have reorded these two paragraphs but this still needs to be cleaned up. I
% don't think this is quite right.
The simulation model is built in an object-orientated manner using Python.
...
Edges are created or removed whenever the following events take place:

\begin{itemize}
    \item a customer begins service at a server
    \item a customer becomes blocked
    \item a customer leaves a server
\end{itemize}

% This paragraphs below doesn't make much sense in connection to the previous
% one?
...
% It feels like something is missing here\dots
...
% Are you using Markov or markov? Should be Markov throughout
In this section Markov models are built for the following deadlocking queueing networks, and their expected time to deadlock found:
...
In general a continuous Markov chain model of a deadlocking queueing network is
defined by a set of states $s \in S$ and the transition rates between these
states $q_{s_1,s_2}$.  Each state $s$ uniquely defines a configuration of
customers around the queueing network.  Deadlocked states are also present,
either denoted by that specific configuration of customers, or by negative
numbers, for example $-1$.  Deadlocked states cannot transition to any other
state, and so are absorbing states of the Markov chain.  Therefore any queueing
network that can experience deadlock is guaranteed to experience deadlock, as
absorbing Markov chains are guaranteed to enter one of its absorbing states.
...
% My spell checker says that discretising is not a word: check :)
...
by the $(i, j)^{\text{th}}$ element of the follow matrix:
\begin{equation}
  (I - T)^{-1}U
\end{equation}
...
% Use not bitmap for your images: these are png right?
\begin{center}
\begin{figure}[!htbp]
\begin{center}
...
Figure~\ref{fig:decomposeqnet} shows how $\Omega$ contains, and is made up from, $\Omega_{1_1}$, $\Omega_{1_2}$ and $\Omega_2$.
...
\begin{subfigure}[b]{0.5\textwidth}
  ...
  \caption{Scatter plot of $p_{(-1)}$ against $E$.}
  \label{fig:scatter_p1}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/scatter_p3_ratio_p}
  \caption{Scatter plot of $p_{(-3)}$ against $E$.}
