...
In Type III blocking (repetitive services, RS-FD and RS-RD, rejection blocking) instead of getting blocked a customer is required to repeat their service if there is no capacity at their destination. This type of blocking comes in two flavours, fixed destination where the customer's destination does not change at each repetition of service, and random destination, where the customer's destination is re-sampled from a probability distribution after each repetition.
...
In order to detect when deadlock has occurred in a queueing network simulation, the state digraph is used.
...
\begin{itemize}
  ...
  \item Open two node, multi-server restricted queueing network with routes between nodes. (Section~\ref{sec:2nodeMS})
\end{itemize}
...
Let $p_{(-1)}$, $p_{(-2)}$ and $p_{(-3)}$ denoted the probabilities of $\Omega$ reaching deadlocked state (-1), (-2) and (-3) respectively, calculated using Equation~\ref{eq:abs_probs}.

Figures~\ref{fig:scatter_p1} and \ref{fig:scatter_p3} show scatter plots of $p_{(-1)}$ and $p_{(-3)}$ against the ratio $E$ respectively (each taking a random sample of 100000 data points for ease of graphing).
...
The same conclusion can be deduced for $p_{(-2)}$ as the mechanics of the $\Omega$ system is symmetrical.
