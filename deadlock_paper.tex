...
\RUNAUTHOR{Palmer, Harper \& Knight}

% Title or shortened title suitable for running heads. Sample:
% \RUNTITLE{Bundling Information Goods of Decreasing Value}
% Enter the (shortened) title:
\RUNTITLE{On Deadlock in Queueing Networks}

% Full title. Sample:
% \TITLE{Bundling Information Goods of Decreasing Value}
% Enter the full title:
\TITLE{On Deadlock in Queueing Networks}
...
\AFF{School of Mathematics, Cardiff University, } %, \URL{}}
% Enter all authors
} % end of the block
...
\KEYWORDS{queueing networks, deadlock}
...
state-graphs and other variants (\cite{cheng90, elmagarmid86, coffmanelphick71, choetal95}).
...
queueing networks in and out of deadlock, and the corresponding state digraph in each case.
...
And so there is no path from $X$ to a vertex with out-degree of 0. \Halmos
...
\begin{proof}{Proof}
  ...
  The resulting state digraph in Figure~\ref{fig:counter_example_2} has a weakly connected component with a sink. \Halmos
  \end{enumerate}
\end{proof}
...
defined by a set of states $S$ and the transition rates between these states
$q_{s_1,s_2}$ for \(s_1, s_2 \in S\).
Each state $s\in S$ uniquely defines a configuration of customers in the queueing network.
...
probability $r_{11}$ results in reaching deadlock sooner.
...
\omega &\leq \min(\omega_{1_1}, \omega_{1_2}, \omega_2) \Halmos
\end{align*}

\end{proof}

This bound has been tested for over 7 million parameter sets. A sample of the data can be found here \url{http://www.geraintianpalmer.org.uk/Assets/deadlock_bound_data.csv}.
In order to gain an understanding of how loose or tight this bound is, the
following ratio is analysed:
...
\omega_2)$, have: $0 \leq \beta \leq 1$.
If $\beta$ is close to 1 then the bound is tight, and if $\beta$ is close to 0 then there is a large percentage difference between the bound and the actual value for $\omega$.
...
are less than 1), it not, in general, tight, with some parameter sets giving a value of $\beta$ as low as $1.98 \times 10^{-5}$.
