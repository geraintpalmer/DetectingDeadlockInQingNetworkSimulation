...
The function $\omega(\mu)$ that describes the expected time to deadlock of
an this system as the service rate $\mu$ varies, and all other parameters are
fixed, has one critical point and is a local minimum for $\mu \in (0, \infty)$.
\end{remark}

The behaviour of $\omega(\mu)$ can be interpreted as follows:
\begin{itemize}
\item At $\lim_{\mu \to 0} \omega(\mu)$ there is infinite service time,
and so infinite time until deadlock.
\item At $\lim_{\mu \to \infty} \omega(\mu)$ there is zero service time,
...
The following remark identifies the maximum of $\omega(\mu)$ on a closed
interval.\\

\begin{remark}\label{rem:findmaximum}
$\arg\max_{\mu \in [a, b]} \omega(\mu) \in \{a, b\}$.

From the closed interval method of finding absolute maximum \cite{tan09}, the
absolute maximum of $\omega(\mu)$ on the closed interval $[a, b]$ is either
...
$\omega(\hat{\mu}) \leq \omega(a)$ and
$\omega(\hat{\mu}) \leq \omega(b)$.
Therefore $\omega(\mu)$ obtains its maximum at either $a$ or $b$.
\end{remark}
