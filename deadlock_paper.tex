...
In this section Markov models are build for the Networks~\ref{itm:1Nss}-\ref{itm:2Nmss}, and their expected time to deadlock found.
Due to the importance of the order of customers getting blocked, and so the order in which unblockages happen, it was found that the state space for Network~\ref{itm:2Nmsf} was too large to model in a similar way to the others. As a customer can either get blocked to Node 1 or Node 2, then every possible ordering of blocked customers and their destination nodes is required for each node.
...
Figures~\ref{fig:scatter_p1} and \ref{fig:scatter_p3} show scatter plots of $p_{(-1)}$ and $p_{(-3)}$ against the ratio $\beta$ respectively (each taking a random sample of 100000 data points for ease of graphing).
...
\begin{figure}[!htbp]
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/scatter_p1_ratio_png}
  \caption{Scatter plot of $p_{(-1)}$ against $\beta$.}
  \label{fig:scatter_p1}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/scatter_p3_ratio_png}
  \caption{Scatter plot of $p_{(-3)}$ against $\beta$.}
  \label{fig:scatter_p3}
\end{subfigure}
\caption{Scatter plots of absorption probabilities against $\beta$.}
