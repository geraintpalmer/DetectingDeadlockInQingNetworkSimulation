...
customer with server $A_1$ and the customer with server $B_1$) who are blocking
eachother only.
...
An understanding of how deadlock behaves in these models will aid in the
modelling process.
...
In Type III blocking instead of getting blocked a customer is required to repeat
their service if there is no capacity at their destination.
...
as Discrete Event Systems (DES) \cite{reveliotis15a, reveliotis15b}.
...
marchettimunierkordon09, belik90}.
...
when blocking of Type I occurs \cite{kunduakyildiz89, liebeherrakyildiz95,
onvural90, schmidtjackman00}
These have involved determining the minimum queueing space assignment that
prevents deadlock for a given population size, and turning customers away if
