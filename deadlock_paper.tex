...
\begin{abstract}
In this paper deadlock in open restricted queueing networks is explored.
Markov models of five deadlocking queueing networks are given, and their properties on expected time to deadlock found.
The five networks are a one node single server, one node multi server, two node single server without loops, two node multi server without loops, and two node single server with loops.
These properties are compared to results obtained using the simulation model, where a state digraph is used to terminate the simulation once deadlock is reached.
A bound on the time to deadlock of the two node single server with loops network is derived by comparing properties with other queueing networks that are embedded within it.
\end{abstract}
...
Restricted queueing networks that exhibit blocking are well discussed in the literature, both exact \cite{hunt56, baber08, aviitzhakyadin65, koizumietal05, latoucheneuts80} and approximate methods \cite{takahashi80, korporaaletal00}. Discussion on restricted queueing networks with feedback loops, that may exhibit deadlock, are sparse however.
...
Deadlock prevention has been discussed for queueing networks.
...
The literature does not discuss deadlock prevention in open restricted queueing networks.

\subsection{Deadlock Avoidance}

There are algorithms discussed in the literature for the dynamic avoidance of deadlock in general systems.
...
\begin{figure}[!htbp]
  \begin{center}
  \includestandalone[width=0.5\textwidth]{images/exampledigraph_in}
  \end{center}
...
\begin{figure}[!htbp]
  \begin{center}
  \includestandalone[width=0.5\textwidth]{images/exampledigraph_out}
  \end{center}
...
The only critical point in $(a, b)$ is $\hat{\mu}$, and is a local minimum (from Lemma~\ref{lem:oneminima}), and so $\hat{\mu} \leq a$ and $\hat{\mu} \leq b$.
...
As time to deadlock does not increase monotonically with service rate $\widetilde{\Omega}_{1_1}$ must be compared with either the system with the smallest service rate or the largest, whichever yields the longest time to deadlock.
...
Therefore using Lemma~\ref{lem:oneminima} and Remark~\ref{rem:findmaximum} $\widetilde{\omega}_{1_1}$ is compared $\omega_{1_1} = \max(\omega_{1_1}^{\star}, \omega_{1_1}^{\star\star})$.

Comparing this system to $\widetilde{\Omega}_{1_1}$: the effective arrival rate in $\widetilde{\Omega}_{1_1}$ is greater than or equal to the effective arrival rate in that system; this is due to the extra customers who have transitioned from the Node 2 to Node 1. An increase in the arrival rate causes the mean time to deadlock to decrease.

It is concluded that $\widetilde{\omega}_{1_1} \leq \omega_{1_1}$ at $\widetilde{\omega}_{1_1}$'s worst case scenrio, and so $\widetilde{\omega}_{1_1} \leq \omega_{1_1}$ in general.

A similar argument yields $\widetilde{\omega}_{1_2} \leq \omega_{1_2}$.
