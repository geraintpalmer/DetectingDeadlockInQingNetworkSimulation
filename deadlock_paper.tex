...
Due to the importance of the order in which customers get unblocked, it was found that the state space for Network~\ref{itm:2Nmsf} was too large to model in a similar way to the others. This is further discussed in Section~\ref{sec:conclusions}.
...
Combining the two requirements above, it is clear that as the number of servers increases, the state space for this queueing network quickly grows combinatorially, and must denote the state space is a different format to the networks considered in this paper.
