...
r12, r21 = 0.25, 0.15
L1, L2 = 4.0, 5.0
directory = '/Users/geraintianpalmer/Documents/DetectingDeadlockInQingNetworkSimulation/data_for_graphs/2NodeSimple_newformulation/run_10000_itrs/vary_n2/'
...
class Network:
    ...
    def __init__(self, n1, n2, mu1, mu2, r12, r21, L1, L2):
        ...
        self.L1 = L1
        self.L2 = L2
        self.State_Space = [((k1, b1), (k2, b2)) for k1 in range(self.n1+2) for b1 in range(2) for k2 in range(self.n2+2) for b2 in range(2) if b1*(self.n2+1)<=k2 if b2*(self.n1+1)<=k1 if k1+b1<= self.n1+1 if k2+b2<=self.n2+1] + [((self.n1, 1), (self.n2, 1))]
    ...
    def find_transition_rates(self, state1, state2):
        ...
        Finds the transition rates for given state transition
            ...
            0
        """
        delta = ((state2[0][0]-state1[0][0], state2[0][1]-state1[0][1]), (state2[1][0]-state1[1][0], state2[1][1]-state1[1][1]))
        if delta == ((1, 0), (0, 0)):
            if state1[0][0]+state1[0][1]<=self.n1:
                return self.L1
            return 0
        if delta == ((0, 0), (1, 0)):
            if state1[1][0]+state1[1][1]<=self.n2:
                return self.L2
            return 0
        if delta == ((-1, 0), (0, 0)) or delta == ((0, 0), (0, -1)):
            if state1[0][1] == 0:
                return (1-self.r12)*self.mu1
        if delta == ((0, 0), (-1, 0)) or delta == ((0, -1), (0, 0)):
            if state1[1][1] == 0:
                return (1-self.r21)*self.mu2
        if delta == ((-1, 0), (1, 0)) or delta ==((0, 0), (1, -1)):
            if state1[1][0]+state1[1][1]<= self.n2 and state1[0][1]==0:
                return self.r12*self.mu1
            return 0
        if delta == ((-1, 1), (0, 0)):
            return self.r12*self.mu1
        if delta == ((1, 0), (-1, 0)) or delta == ((1, -1), (0, 0)):
            if state1[0][0]+state1[0][1]<=self.n1 and state1[1][1]==0:
