...
\begin{proposition}\label{prop:number_deadlocks}
...
The following proposition gives the number of different deadlocked states that can reached at first instance.
First recall that Stirling numbers of the second kind $S(n, k)$ give the number of ways to partition a set of $n$ elements into $k$ subsets.\\

\begin{proposition}
  If the queueing network $Q$ with $N$ nodes is complete, and every node has finite queueing capacity, then the number of possible deadlocked states that can be reached at first instance is $S(N, 2)$, where $S(n, k)$ denotes a Stirling number with parameters $n$ and $k$.
\end{proposition}

\begin{proof}

\begin{itemize}
\item Define $S_N$, $D_N$, $P_N$, $\xi$ and $g$ as in Proposition~\ref{prop:number_deadlocks}.
\item Define $F_N$ as the set of possible deadlocks that can be reached at first instance on a queueing network $Q$ with $N$ nodes.
\item Define $T_N$ as the number of ways $S_N\cup\{\xi\}$ can be partitioned into two subsets.
\end{itemize}

Note that $F_N \subset D_N$ and $T_N \subset P_N$.
By definition $|T_N| = S(N, 2)$.

Noting that a deadlocked state that can be reached at first instance only has one deadlocked component then the following must be true: if $y \in F_N$ then $g(y)$ is a partition of two subsets, one subset containing all the nodes in the sole deadlocked component, and one subset containing all undeadlocked nodes.

Thus $g$ maps $F_N \to T_N$.
Recall that $g$ is both 1-1 and onto, thus $|F_N| = |T_N| = S(N, 2)$. 
\end{proof}
...
\subsection{Performance of the Bound}
This bound was tested on 7.2 million parameter sets of the $\Omega$ system.
...
Figure~\ref{fig:bound_ratio_hist} shows a histogram of $E$ for 7.2 million parameter sets.
...
To investigate which parameters effect $E$, and so which parameters effect the tightness of the bound, further analysis is needed.

\begin{figure}[!htbp]
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/ratio_actual_n1s}
  \caption{Effect of $n_1$ on $E$.}
  \label{fig:ratio_ns}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/ratio_actual_rho_ratios}
  \caption{Effect of $\rho^{\star}$ on $E$.}
  \label{fig:ratio_rho_ratio}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/ratio_actual_rs}
  \caption{Effect of $r^{\star}$ on $E$.}
  \label{fig:ratio_rs}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/ratio_actual_bound_p3}
  \caption{Effect of $p_{(-3)}$ on $E$.}
  \label{fig:ratio_p3s}
\end{subfigure}
\caption{Preliminary analysis of parameters on $E$.}
\label{fig:prelim_ratio_analysis}
\end{figure}

Some preliminary analysis is presented in Figure~\ref{fig:prelim_ratio_analysis}.
Figure~\ref{fig:ratio_ns} shows a stacked plot of the estimated pdf of $E$ split by the parameter $n_1$.
It can be seen that although $E$ has the same range for each $n_1$, the mode and mean shift to the right as $n_1$ increases.
This suggests that the bound is tighter for parameter sets with a larger queueing capacities.

Figure~\ref{fig:ratio_rho_ratio} shows the effect of the relative difference in traffic intensities of each node on $E$.
Again the y-axis has been normalised.
The groups are groupes by $\rho^{\star}$, defined by $\rho^{\star} = \max(\frac{\rho_1}{\rho_2}, \frac{\rho_2}{\rho_1}) = \max(\frac{\Lambda_1\mu_2}{\Lambda_2\mu_1}, \frac{\Lambda_2\mu_1}{\Lambda_1\mu_2})$.
If this measure $\rho^{\star}$ is close to 1 then both nodes have similar traffic intensities, however the larger this measure the larger the difference in the business of the nodes.
It can be seen that when the nodes have similar traffic intensities the bound is less tight that when the traffic intensities are very different.

Figure~\ref{fig:ratio_rs} shows the effect of the transition probabilities on $E$.
This time the histograms are overlayed on top of eachother rather than stacked, and the y-axis has been normalised for each histogram as some groups have vastly different sample sizes to others.
The histograms are split by $r^{\star} = \min(r_{12}, r_{21})$, that is grouping high values of both $r_{12}$ and $r_{21}$ together.
This split was chosen to highlight the difference in $E$ as the probability to deadlocked state (-3) increased.
It can be seen that higher $r^{\star}$ values yield higher $E$ values in general.
This suggests that the bound is tighter for parameter sets with high probability if reaching deadlocked state (-3).

Figure~\ref{fig:ratio_p3s} shows the effect of $p_{(-3)}$, the probability of reaching deadlocked state (-3), of the performance of the bound.
It can clearly be seen that the bound is much tighter for queueing networks with a high $p_{(-3)}$ than those with a lower probability to deadlcoked state (-3).
...
In order to determine for which parameter sets the bound's tightness is good, average or bad, a k-means clustering algorithm was used on the $E$ values.
This partitioned the observations into 3 clusters:

\begin{itemize}
  \item Group A in which the bound's tightness is bad.
  \item Group B in which the bound's tightness is average.
  \item Group C in which the bound's tightness in good.
\end{itemize}

Figure~\ref{fig:clustered_ratio} shows these three clusters partitioning the histogram of $E$.

\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width=0.5\textwidth]{images/ratio_actual_bound_clusters}
  \end{center}
  \caption{Histogram of $E$, with clusters.}
  \label{fig:clustered_ratio}
\end{figure}

Preliminary analysis seemed to indicate that the bound's tightness is related to the absorption probabilities, especially $p_{-3}$.
...
\begin{figure}[!htbp]
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/scatter_p1_ratio}
  \caption{Scatterplot of $p_{(-1)}$ against $E$.}
  \label{fig:scatter_p1}
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
  \includegraphics[width=\textwidth]{images/scatter_p3_ratio}
