...
Thus increasing the service rate removes people fromå the system, and as such there is less chance of getting blocked and deadlocked.
\end{itemize}
...
\begin{itemize}
  ...
  \item Define $\Omega_{1_1}^{\star\star}$ as the 1 node queueing network described in Subsection~\ref{sec:1nodenet} with the parameter set $\{\Lambda_1$, $\min(\frac{1}{2}\mu_1, m)$, $n_1$, $r_{11}\}$. Let its mean time to deadlock be denoted by $\omega_{1_1}^{\star\star}$.
  ...
  \item Define $\Omega_{1_2}^{\star\star}$ as the 1 node queueing network described in Subsection~\ref{sec:1nodenet} with the parameter set $\{\Lambda_2$, $\min(\frac{1}{2}\mu_2, m)$, $n_2$, $r_{22}\}$. Let its mean time to deadlock be denoted by $\omega_{1_2}^{\star\star}$.
...
\end{itemize}
where $m = \frac{1}{\frac{1}{\mu_1} + \frac{1}{\mu_2}}$.
...
\begin{itemize}
  ...
  \item Let $\widetilde{\Omega}_{1_2}$ denote the $\Omega_{1_2}$ sytem within $\Omega$. Let $\widetilde{\omega}_{1_2}$ denote the mean time to deadlock of $\widetilde{\Omega}_{1_2}$.
  \item Let $\widetilde{\Omega}_2$ denote the $\Omega_2$ sytem within $\Omega$. Let $\widetilde{\omega}_2$ denote the mean time to deadlock of $\widetilde{\Omega}_2$.
\end{itemize}
...
\begin{itemize}
  \item If neither Node 1 nor Node 2 is blocked, then the effective service rate is equal to $\mu_1$.
  \item If Node 2 is blocked (due to a customer wanting to transition to Node 1), then when Node 1 has a service it simultaneously accepts a customer from Node 2. There is no change in state until the next customer finishes service. Therefore Node 1 remains in the same state for two customers' service times; and so we can imagine this process as one service with effective service rate $\frac{1}{2}\mu_1$.
  \item If Node 2 is full but not blocked, and Node 1 finishes service and the customer wants to transition to Node 2, then that customer becomes blocked. That customer remains blocked until there is room at Node 2, that is when Node 2 has a service. Therefore that customer's effective service time is his service time plus what remains of the service time of the customer at Node 2. As all service times are Markovian it does not matter how long a customer has been in service at Node 2, the expected time until its unblockage is $\frac{1}{\mu_1} + \frac{1}{\mu_2}$, and the effective service rate is $m = \frac{1}{\frac{1}{\mu_1}+\frac{1}{\mu_2}}$.
\end{itemize}
...
The largest service rate is clearly $\mu_1$, corresponding to system $\Omega_{1_1}^{\star}$ and so the smallest must be $\min(\frac{1}{2}\mu_1, m)$ corresponding to $\Omega_{1_1}^{\star\star}$.
