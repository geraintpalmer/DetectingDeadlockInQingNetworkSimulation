...
\begin{tabular}{ p{5cm} p{10cm} }
  ...
  Knot                         & In a directed graph, a knot is a set of vertices such that while traversing the directed edges of that directed graph, once a vertex in the knot is reached, you cannot rech any vertex that is not in the knot.\\
\end{tabular}
...
A deadlocked state arises at time $t$ if and only if there exists a weakly connected component of $D(t)$ that doesn't contain a sink vertex.
...
\begin{itemize}
    ...
    \item Nodes causing deadlock: These are nodes where every server is occupied by a blocked individual, and there is at least one blocked individual waiting to enter that node who is directly or indirectly being blocked by an individual in this node.
    \item Nodes affected by deadlock: These are nodes containing at least one blocked individual who is directly or indirectly being blocked by an individual at a node that is causing deadlock, but is not classified as causing deadlock itself.
\end{itemize}

At the first instance of deadlock, consider the weakly connected component $G$ of $D(t)$ that contains no sink nodes.
The vertices of $G$ correspond to servers.
As there is no sink node, all vertices of $G$ have an out-edge, and so all vertices in $G$ contain a blocked individual.
Therefore, there are no vertices in $G$ belonging to nodes that are not deadlocked.

A node is causing deadlock if all its servers belong to a knot in $G$.
A node that is affected by deadlock has no servers belonging to a knot in $G$.
