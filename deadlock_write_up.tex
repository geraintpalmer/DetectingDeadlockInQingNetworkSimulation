...
This work will investigate deadlocking in open restricted queueing networks.
Section~\ref{sec:litreview} will give an overview of the literature on both resticted queueing networks and general deadlocks.
Section~\ref{sec:typesofdeadlock} will differentiate between two different types of deadlocks in queueing networks.
In Section~\ref{sec:detectdeadlock} the state digraph will be defined, explained, and it will be shown how to use the state digraph to detect deadlock in queueing network simulations.
Section~\ref{sec:markovmodels} Markov chain models of five simple queueing networks will be given and their deadlocking properties explored.
Finally a bound will be given on the expected time to deadlock of one of the queueing network models given.
...
The following definition of deadlock will be used.\\
...
If the routing matrix of $Q$ is complete, that is there is a possible route from every service station to every other service station, and every node has finite queueing capacity, then the following proposition gives the number of possible deadlocked states.\\

\begin{proposition}
  If the routing matrix of a queueing network $Q$ with $N$ nodes is complete, that is there is a possible route from every service station to every other service station, and every node has finite queueing capacity, then the number of possible deadlocked states is $B_{N+1} - 1$, where $B_n$ denotes the $i$th Bell number.
\end{proposition}

\begin{proof}
  The number of different types of deadlock correspond to the number of combinations of cycles in the routing matrix, including self loops.
  As there is a route from every node to every other node, this corresponds to every possible combination of nodes in deadlock, for every combination of deadlocks.
  Let $S$ denote the set of nodes.

  A Bell number $B_n$ is the number of ways a set of $n$ elements can be partitioned into non-empty subsets.
  They are generated using the following recurrance relation
  ...
  At first glance a Bell number seems inappropriate as this problem isn't looking for every possible partition, but every possible partition of combinations of nodes, to allow for partitions that are inexhaustive of the set.
  This is required as not every node will be in deadlock.

  The set of sets of all nodes in deadlock is $\mathcal{P}(S) \setminus \emptyset$, and every set of deadlocked nodes $S_k$ has $B_{\left| S_k \right|}$ possible partitions.


  In $\mathcal{P}(S)$ there are $\binom{N}{i}$ sets of size $i$.
  And so the number of ways to partition every set in $\mathcal{P}(S)$ is:
  ...
  Only one of these partitions corresponds to the empty set, that is when no nodes are deadlocked.
  Therefore the number of daedlocked states correspond to $B_{N+1}-1$.
