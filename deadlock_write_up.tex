...
If the routing matrix of $Q$ is complete, that is there is a possible route from every service station to every other service station, then the following proposition gives the number of possible deadlocked states.\\

\begin{proposition}
  If the routing matrix of a queueing network $Q$ with $N$ nodes is complete, that is there is a possible route from every service station to every other service station, then the number of possible deadlocked states is $B_{N+1} - 1$, where $B_n$ denotes the $i$th Bell number.
\end{proposition}

\begin{proof}
  The number of different types of deadlock correspond to the number of cycles in the routing matrix, including self loops.
  As there is a route from every node to every other node, this corresponds to every possible combination of nodes in deadlock.
