...
The state graph $D(t)$ can be partitioned into $N$ subgraphs, $d_1(t), d_2(t), ..., d_N(t)$, where the vertices of $d_i(t)$ represent the servers of node $i$.
...
Also note that each of the defined subgraphs $d_i(t)$, are empty graphs on $c_i$ vertices; that is they contain $c_i$ nodes and no edges.
Every vertex in $d_i(t)$ has the same ancestors.\newline
